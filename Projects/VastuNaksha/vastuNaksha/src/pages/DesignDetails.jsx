...
export default function DesignDetails() {
  ...
  const sendWhatsAppMessage = () => {
    const phoneNumber = "917070580889"; // +91 7070580889
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <div className="relative h-96 md:h-[500px] rounded-2xl overflow-hidden bg-gray-200 shadow-lg">
              <img
              ...
              <div className="absolute top-6 left-6 z-10">
                <span className="inline-block px-4 py-2 bg-black/80 text-white text-sm font-semibold rounded-full backdrop-blur-sm">
                  {design.category}
                </span>
              </div>
              ...
              {imageVariations.length > 1 && (
                <>
                  <button
                    onClick={() =>
                      setCurrentImageIndex((prev) =>
                        prev === 0 ? imageVariations.length - 1 : prev - 1
                      )
                    }
                    className="absolute left-6 top-1/2 -translate-y-1/2 z-10 p-3 bg-white/90 hover:bg-white rounded-full shadow-lg transition-all transform hover:scale-110"
                  >
                    <svg
                  ...
                  <button
                    onClick={() =>
                      setCurrentImageIndex((prev) =>
                        prev === imageVariations.length - 1 ? 0 : prev + 1
                      )
                    }
                    className="absolute right-6 top-1/2 -translate-y-1/2 z-10 p-3 bg-white/90 hover:bg-white rounded-full shadow-lg transition-all transform hover:scale-110"
                  >
                    <svg
                      className="w-6 h-6 text-black"
                      fill="none"
          ...
          <div className="space-y-8">
            <div>
              ...
              {design.size && (
                <div className="inline-block px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold mb-6">
            ...
            <div className="flex flex-col gap-4 pt-8">
              <button
                ...
                className="w-full px-8 py-4 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3 text-lg"
              >
                <svg
                  className="w-6 h-6"
                  viewBox="0 0 24 24"
              ...
              <button
                onClick={() => navigate("/")}
                className="w-full px-8 py-4 border-2 border-black text-black hover:bg-black hover:text-white rounded-xl font-semibold transition-all transform hover:scale-105 active:scale-95"
              >
                Back to Designs
              </button>
            </div>
